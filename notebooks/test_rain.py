"""
Visual test — apply all three rain intensities to the first frame of every clip
...
for video_path in videos:
    ...
    light = apply_rain(frame, "light")
    medium = apply_rain(frame, "medium")
    heavy = apply_rain(frame, "heavy")

    top = np.hstack([frame, light])
    bot = np.hstack([medium, heavy])
    grid = np.vstack([top, bot])

    out_path = OUT_DIR / f"{video_path.stem}_rain_grid.jpg"
    save_frame(grid, out_path)
    print(f"✓ {video_path.name} → {out_path}")

print("\nDone. Open outputs/aug_rain/ to inspect.")